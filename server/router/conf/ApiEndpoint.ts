export enum ApiEndopoint {
    FRONT_USER_INFO = "/videomng/v1/frontuserinfo",
    FRONT_USER_INFO_ID = "/videomng/v1/frontuserinfo/:id",
    FRONT_USER_LOGIN = "/videomng/v1/frontuserlogin",
    FRONT_USER_LOGOUT = "/videomng/v1/frontuserlogout",
    FRONT_USER_CHECK_AUTH = "/videomng/v1/frontusercheckauth",
    VIDEO_INFO = "/videomng/v1/videoinfo",
    VIDEO_INFO_ID = "/videomng/v1/videoinfo/:id",
    VIDEO_COMMENT_ID = "/videomng/v1/video/:videoId/comment",
    FAVORITE_VIDEO_COMMENT = "/videomng/v1/favoritevideo/:videoId/comment",
    FAVORITE_VIDEO = "/videomng/v1/favoritevideo",
    FAVORITE_VIDEO_ID = "/videomng/v1/favoritevideo/:id",
    FAVORITE_VIDEO_MEMO = "/videomng/v1/favoritevideo/:videoId/memo",
    FAVORITE_VIDEO_MEMO_ID = "/videomng/v1/favoritevideo/:videoId/memo/:memoId",
    SEARCH_COMMENT_BY_KEYWORD = "/videomng/v1/video/:videoId/searchcommentbykeyword",
    BLOCK_COMMENT = "/videomng/v1/favoritevideo/:videoId/blockcomment",
    BLOCK_COMMENT_ID = "/videomng/v1/favoritevideo/:videoId/blockcomment/:commentId",
    FAVORITE_COMMENT = "/videomng/v1/favoritevideo/:videoId/favoritecomment",
    FAVORITE_COMMENT_ID = "/videomng/v1/favoritevideo/:videoId/favoritecomment/:commentId",
    VIDEO_CATEGORY = "/videomng/v1/videocategory",
    VIEW_STATUS = "/videomng/v1/viewstatus",
    TAG_INFO = "/videomng/v1/taginfo",
    FAVORITE_VIDEO_TAG_ID = "/videomng/v1/favoritevideo/:videoId/tag",
    FRONT_USER_PASSWORD_ID = "/videomng/v1/frontuserpassword/:id",
    FAVORITE_VIDEO_SORT = "/videomng/v1/favoritevideosort",
    CHANNEL_VIDEO_INFO_ID = "/videomng/v1/channelvideo/:id",
    FAVORITE_VIDEO_CSV_DOWNLOAD = "/videomng/v1/favoritevideo-download-csv",
    FAVORITE_VIDEO_CSV_UPLOAD = "/videomng/v1/favoritevideo-upload-csv",
    HEALTH = "/videomng/v1/health",
    FAVORITE_VIDEO_CUSTOM = "/videomng/v1/favoritevideo/:videoId/custom",
    FOLDER = "/videomng/v1/folder",
    FOLDER_ID = "/videomng/v1/folder/:folderId",
    FAVORITE_VIDEO_FOLDER = "/videomng/v1/folder/:folderId/favoritevideo",
    FAVORITE_VIDEO_FOLDER_ID = "/videomng/v1/folder/:folderId/favoritevideo/:videoId",
    FOLDER_SHARE_VIDEOS = "/videomng/v1/folder/:folderId/shared-videos",
    REFRESH = "/videomng/v1/refresh",
    RECENT_SEARCH_WORD = "/videomng/v1/recentsearchword",
    RECENT_SEARCH_WORD_ID = "/videomng/v1/recentsearchword/:id",
    FREQUENT_SEARCH_WORD = "/videomng/v1/frequentsearchword",
    FREQUENT_SEARCH_WORD_ID = "/videomng/v1/frequentsearchword/:id",
    FAVORITE_SEARCH_WORD = "/videomng/v1/favoritesearchword",
    FAVORITE_SEARCH_WORD_ID = "/videomng/v1/favoritesearchword/:id",
}